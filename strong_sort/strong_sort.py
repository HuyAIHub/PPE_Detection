...
class StrongSORT(object):
    def __init__(self, model_weights, device, fp16, max_dist=0.2,max_iou_distance=0.7,max_age=70, n_init=3,nn_budget=100,mc_lambda=0.995,ema_alpha=0.9):
        self.model = ReIDDetectMultiBackend(weights=model_weights, device=torch.device(device), fp16=fp16)
    ...
    def update(self, bbox_xywh, confidences, classes, ori_img):
        ...
        for track in self.tracker.tracks:
            ...
            conf = track.conf
            outputs.append(np.array([int(x1), int(y1), int(x2), int(y2), track_id, class_id, conf]))
        if len(outputs) > 0:
